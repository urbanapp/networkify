...
## Architecture
- **Vite + vanilla JS ES modules** — no framework. `index.html` is a slim shell; all logic lives in `src/`.
...
- GitHub Actions (`.github/workflows/deploy.yml`) builds and deploys to GitHub Pages on every push to `main` → `urbanapp.github.io/networkify` + custom domain `networkify.app` (DNS: Cloudflare).
